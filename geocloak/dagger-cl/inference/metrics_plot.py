...
y_limits = {"MSE": (0, 1.5), "RMSE": (0, 1.5), "MAE": (0, 1)}
...
for i, metric in enumerate(metrics):
    ...
    ax.yaxis.set_major_locator(
        MaxNLocator(nbins=5)
    )  # Set to a maximum of 5 ticks within the limits
    ...
    ax.legend()
    ax.grid(
        True, color="gray", linestyle="--", axis="y"
    )  # Set grid color and only display horizontal grid lines
...
for i, metric in enumerate(metrics):
    ...
    ax.bar(
        "Val", val_sd, color=colors[i], alpha=0.6, hatch="//", label=f"{metric} Val SD"
    )
