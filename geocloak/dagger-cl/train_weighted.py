...
dataloader_train = DataLoader(ds_train, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, pin_memory=True,  shuffle=True)
...
class EWC:
    ...
    def _diag_fisher(self):
        ...
        for inputs, targets, mask_target, kp_index in self.dataloader:
            ...
            loss = WeightedMSELoss()(outputs[mask_target], targets[mask_target], kp_index).to(device)
            loss.backward()
...
def evaluate_and_log(model, data_loader, criterion, stage, dataset_name):
    ...
    with torch.no_grad():
        for inputs, targets, mask_target, kp_index in data_loader:
            ...
            loss = criterion(outputs[mask_target], targets[mask_target], kp_index).to(device)
...
def train_and_evaluate(model, ewc, train_loader, val_loader, test_loader, epochs=25, stage=1):
    ...
    for epoch in range(epochs):
        model.train()
        total_loss = 0.
        val_loss, val_rmse, test_loss, test_rmse, avg_train_loss, avg_train_loss = 0., 0., 0., 0., 0., 0.
        predictions_list = []
        targets_list = []
        ...
        avg_train_rmse = compute_rmse(np.array(predictions_list), np.array(targets_list))

        # Evaluate
        val_loss, val_rmse = evaluate_and_log(model, val_loader, criterion, stage, "Validation")
        test_loss, test_rmse = evaluate_and_log(model, test_loader, criterion, stage, "Test")
...
def save_model_and_fisher(model, ewc, stage):
    ...
    torch.save({'fisher': fisher_dict, 'means': means_dict}, f'models_new/fisher_stage_{stage}.pth')
...
def load_model_and_fisher(model, stage):
    ...
    fisher_dict = {n: torch.tensor(p).to(model.parameters().__next__().device) for n, p in fisher_info['fisher'].items()}
    means_dict = {n: torch.tensor(p).to(model.parameters().__next__().device) for n, p in fisher_info['means'].items()}
...
ewc = None
best_loss, best_model_state = train_and_evaluate(model, ewc, dataloader_train, dataloader_val, dataloader_test)
...
for stage, ds in enumerate(cl_train_ds):  # Assuming 3 stages
    ...
    # Load new train data
    train_loader_new = DataLoader(ds, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, pin_memory=True, shuffle=True)
    ...
    best_loss, best_model_state = train_and_evaluate(model, ewc, train_loader_new, dataloader_val, dataloader_test)
...
def save_best_metrics_as_csv(best_metrics, filename='models_new/best_metrics_trial.csv'):
...
def plot_bar_metrics():
    ...
    # Bars for losses
    plt.bar(index - 1.5 * bar_width, train_loss, bar_width, label='Train Loss', color='b')
    plt.bar(index - 0.5 * bar_width, val_loss, bar_width, label='Validation Loss', color='g')
    plt.bar(index + 0.5 * bar_width, test_loss, bar_width, label='Test Loss', color='r')
    
    # Bars for RMSE
    plt.bar(index + 1.5 * bar_width, train_rmse, bar_width, label='Train RMSE', color='c')
    plt.bar(index + 2.5 * bar_width, val_rmse, bar_width, label='Validation RMSE', color='m')
