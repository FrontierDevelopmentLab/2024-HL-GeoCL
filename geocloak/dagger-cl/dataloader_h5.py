...
class GeoCLoakDataLoader(Dataset):
    def __init__(self, mapping_file, input_file, target_file_path, target_list, scaler_dir):
        ...
        for tar in target_list:
            self.target[tar+'_geo'] = pd.read_hdf(self.target_file_path+'/'+tar+'_all.h5', key='data')
            self.scaler_db[tar+'_geo'] = self._load_db_scaler("std_"+tar+"_scaler.json")
    ...
    def __getitem__(self, idx):
        #read mapping
            ...
            input_df = self.input.loc[input_ind-dt.timedelta(minutes=90): input_ind].values
            target_dfs = []
