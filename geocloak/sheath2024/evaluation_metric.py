...
def calculate_metrics(predictions, targets, scaler_targets_dir):
    ...
    scaler_targets = load_scaler_from_json(os.path.join(scaler_targets_dir, "scaler_targets.json"))
...
def calculate_individual_metrics(predictions, targets, scaler_targets_dir):
    ...
    individual_metrics = {}
    for i, feature_name in enumerate(['Speed', 'Density', 'Temperature', 'Bt', 'Bx', 'By', 'Bz']):
        feature_rmse = np.sqrt(mean_squared_error(targets_unscaled[:, i], predictions_unscaled[:, i]))
        feature_mae = mean_absolute_error(targets_unscaled[:, i], predictions_unscaled[:, i])
