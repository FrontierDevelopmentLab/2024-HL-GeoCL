...
def train(config):
    ...
    # Define model
    model = SHEATH_MLP(input_dim=26, hidden_dim=config['hidden_dim'], output_dim=7, dropout_rate=config['dropout_rate'])
    model.to(device)
    ...
    for epoch in range(config['epochs']):
        ...
        print(f'Epoch {epoch + 1}/{config["epochs"]}, Train RMSE: {train_rmse}, Val RMSE: {val_rmse}')
