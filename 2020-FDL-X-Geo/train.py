...
from utils.data_utils import get_iaga_data, get_iaga_data_as_list, get_input_data, get_omni_data, load_cached_data, get_wiemer_data
...
def train(config):
    ...
    if (
        ...
        if config['bucketed']:
            ...
            input_data = InputDataset(get_input_data(omni_path="data_local/omni/sw_data.h5",
                                                    ...
                                                    year=yearlist))
            train_idx, test_idx, val_idx, wiemer_idx = generate_indices(
                                                                base="data_local/iaga/",year=yearlist,
                                                                LENGTH=config['past_omni_length'],
                                                                LAG=config['lag'],
    ...
    train_ds, scaler = load_cached_data("cache/train_ds.p", train_idx, None, supermag_data, omni_data, config['targets'], config['past_omni_length'], config['future_length'], config['lag'], bucketed=config['bucketed'])
    test_ds, _ = load_cached_data("cache/test_ds.p", test_idx, scaler, supermag_data, omni_data, config['targets'], config['past_omni_length'], config['future_length'], config['lag'], bucketed=config['bucketed'])
    val_ds, _ = load_cached_data("cache/val_ds.p", val_idx, scaler, supermag_data, omni_data, config['targets'], config['past_omni_length'], config['future_length'], config['lag'], bucketed=config['bucketed'])

    # load weimer data for debugging
    # if os.path.exists("cache/wiemer_ds.p"):
    #     wiemer_ds = pickle.load(open("cache/wiemer_ds.p", "rb"))
    # else:
    #     wiemer_ds = get_wiemer_data(omni_data, supermag_data, config['targets'], scaler, config['lag'], config['past_omni_length'], config['future_length'], wyear="2015", bucketed=config['bucketed'])
