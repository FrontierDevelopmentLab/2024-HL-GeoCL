...
from utils.data_utils import (get_iaga_data_as_list, get_omni_data,
                              get_weimer_data_indices, get_wiemer_data,
                              load_cached_data)
...
def train(config):
    ...
    if (
        ...
        omni_data = OMNIDataset(
            get_omni_data("data_local/omni/sw_data.h5", year=train_yearlist)
        )
        weimer_indices = get_weimer_data_indices(
            targets, lag, past_omni_length, future_length, supermag_data, storm_yearlist
        )
    ...
    train_ds, scaler = load_cached_data(
        "cache/train_ds.p",
        train_idx,
        None,
        supermag_data,
    ...
    if os.path.exists("cache/wiemer_ds.p"):
        ...
        wiemer_ds = get_wiemer_data(
            targets, scaler, lag, past_omni_length, future_length, storm_yearlist
        )
