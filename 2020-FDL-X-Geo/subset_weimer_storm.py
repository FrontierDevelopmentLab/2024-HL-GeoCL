...
txtfile = pd.read_csv(
    f"full_data_panos/symh/symh_{weimer_year}.lst", delim_whitespace=True, header=None
).values
ymd = pd.to_datetime(
    txtfile[:, 1] - 1, unit="D", origin=pd.Timestamp(f"{txtfile[0,0]}-01-01")
)
date_symh = pd.to_datetime(
    {
        "year": ymd.year,
        "month": ymd.month,
        "day": ymd.day,
...
symh_inds = np.argmin(
    np.abs(
        date_symh.to_numpy().reshape([1, -1])
        - pd.to_datetime(weimer_times_unix, unit="s").to_numpy().reshape([-1, 1])
    ),
    axis=-1,
)
...
np.savez(
    f"{subpath}/supermag_omni_data.npz",
    data=data,
    dates=dates,
    features=features,
    omni=omnivalues,
    omni_features=omnifeatures,
    idx=idx,
    symh=symh_value,
    stations=stations,
)
...
tmp_dates = (
    pd.to_datetime(np.array([dates[0], dates[-1]]).reshape(-1), unit="s")
    .to_numpy()
    .reshape([-1, 1])
)
...
np.savez(
    f"{subpath}/f107.npz",
    f107=f107_data[match[0] : match[-1]],
    dates=f107_dates[match[0] : match[-1]],
)
