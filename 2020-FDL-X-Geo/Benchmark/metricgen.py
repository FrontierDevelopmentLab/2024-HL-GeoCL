...
def Confusion_matrix(targdbe, targdbn, preddbe, preddbn):
    ...
    for k in events_targ.keys():
        ...
        falsealarm = np.nansum(
            np.heaviside(events_pred[k] - events_targ[k], 0.0), axis=0
        )
...
def EventMetrics(targdbe, targdbn, preddbe, preddbn):
    ...
    for k in confuse.keys():
        ...
        pc = (tmpconf["hits"] + tmpconf["truenegative"]) / (
            tmpconf["hits"]
            + tmpconf["falsealarm"]
            + tmpconf["misses"]
            + tmpconf["truenegative"]
        )
        hss = 2 * (
            tmpconf["hits"] * tmpconf["truenegative"]
            - tmpconf["misses"] * tmpconf["falsealarm"]
        )
        hss = hss / (
            (tmpconf["hits"] + tmpconf["misses"])
            * (tmpconf["misses"] + tmpconf["truenegative"])
            + (tmpconf["hits"] + tmpconf["falsealarm"])
            * (tmpconf["falsealarm"] + tmpconf["truenegative"])
        )
