...
def train(config):
    ...
    if (
        ...
        supermag_data = SuperMAGIAGADataset(
            *get_iaga_data("data_local/iaga/2015/2015/")
        )
        omni_data = OMNIDataset(
            get_omni_data("data_local/omni/sw_data.h5", year="2015")
        )
    ...
    if overfit:
        ...
        test_ds, _ = load_cached_data(
            "tiny_cache/test_ds.p",
            test_idx,
            scaler,
            supermag_data,
            omni_data,
    ...
    if os.path.exists("cache/wiemer_ds.p"):
        ...
        wiemer_ds = get_wiemer_data(
            targets, scaler, lag, past_omni_length, future_length
        )
