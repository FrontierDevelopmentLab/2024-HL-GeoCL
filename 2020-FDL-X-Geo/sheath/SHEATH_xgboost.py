...
if __name__ == "__main__":
    ...
    ch_net_areas = np.array([np.sum(image)/(image.shape[0]*image.shape[1]) for image in ch_masks])  # Calculate the total area of open-field-line regions    
    ...
    ar_net_areas = np.array([np.sum(image)/(image.shape[0]*image.shape[1]) for image in ar_masks])  # Calculate the total area of open-field-line regions    
