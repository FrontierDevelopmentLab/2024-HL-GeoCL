...
if __name__ == "__main__":
    ...
    predictions = [HSE_gb_model.predict(ds[0], iteration_range=(0, HSE_gb_model.best_iteration + 1)) for ds in evallist]
    np.savez("logs/model_predictions.npz",y_train=y_train,y_val=y_val,y_test=y_test,y_2017=y_2017,y_2015=y_2015,y_2011=y_2011,
             x_train=x_train,x_val=x_val,x_test=x_test,x_2017=x_2017,x_2015=x_2015,x_2011=x_2011,
             predictions_train = predictions[0],predictions_test = predictions[1],predictions_val = predictions[2],
             predictions_2017=predictions[3], predictions_2015=predictions[4], predictions_2011=predictions[5])
             
    labels = [ds[1] for ds in evallist]
    print("Predicting and computing statistics, saving....")
    for preds,targs,label in zip(predictions,[y_train,y_test,y_val,y_2017,y_2015,y_2011],labels):
