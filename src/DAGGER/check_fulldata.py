...
supermag_data = SuperMAGIAGADataset(
    *get_iaga_data_as_list(base="full_data_panos/iaga/", year=yearlist)
)
yearlist = list(np.arange(2010, 2019).astype(str))
omni_data = OMNIDataset(get_omni_data("data_local/omni/sw_data.h5", year=yearlist))

yearlist = list(np.arange(2010, 2019).astype(int))
train_idx, test_idx, val_idx, weimer_idx = generate_indices(
    base="full_data_panos/iaga/",
    year=yearlist,
    LENGTH=past_omni_length,
    LAG=lag,
...
train_ds = ShpericalHarmonicsDatasetBucketized(
    supermag_data,
    omni_data,
    train_idx,
...
print(
    f"OMNI: {pd.to_datetime(v[3][0].ravel(),unit='s')}-- {pd.to_datetime(v[3][-1].ravel(),unit='s')}, supermag: {pd.to_datetime(v[4].ravel(),unit='s')}"
)

val_idx = np.asarray(val_idx)
val_ds = ShpericalHarmonicsDatasetBucketized(
    supermag_data,
    omni_data,
    val_idx,
    f107_dataset="data_local/f107.npz",
...
test_ds = ShpericalHarmonicsDatasetBucketized(
    supermag_data,
    omni_data,
    test_idx,
    f107_dataset="data_local/f107.npz",
...
weimer_ds = ShpericalHarmonicsDatasetBucketized(
    supermag_data,
    omni_data,
    weimer_idx,
    f107_dataset="data_local/f107.npz",
