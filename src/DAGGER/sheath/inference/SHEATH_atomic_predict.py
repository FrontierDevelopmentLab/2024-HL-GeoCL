...
class CloudFetcher:
    def __init__(
        self,
        zarr_bucket="us-fdlx-landing",
        aia_path="fdl-sdoml-v2/sdomlv2_small.zarr",
    ...
    def get_zarr_root(self) -> zarr.hierarchy.Group:
        ...
        """
        print(
            f"Connecting to zarr root in path: {self.aia_path}, {self.hmi_path}, and bucket: {self.zarr_bucket}"
        )

        gcp_zarr = gcsfs.GCSFileSystem(
            project="us-fdl-x",
            bucket=self.zarr_bucket,
            access="read_only",
            requester_pays=True,
        )
        store_aia = gcsfs.GCSMap(
            root=f"{self.zarr_bucket}/{self.aia_path}",
            gcs=gcp_zarr,
            check=False,
            create=True,
        )
        store_hmi = gcsfs.GCSMap(
            root=f"{self.zarr_bucket}/{self.hmi_path}",
            gcs=gcp_zarr,
            check=False,
            create=True,
        )
    ...
    def load_aia_image(self, time, idxs):
        ...
        for wavelength, idx in zip(
            self.aia_wavelengths, idxs
        ):  # The different passbands may have a different index for the same time
...
class AtomicSamurai:
    ...
    def atomic_inference(
        self, aia_timestamp, aia_idx, hmi_timestamp, hmi_idx, scaler_aia
    ):
        """
        ...
        input_datacube = preprocessor.preprocess(
            aia_data,
            hmi_data,
        ...
        results = self.sheathmodule_object.predict(
            xgb.DMatrix(input_datacube),
