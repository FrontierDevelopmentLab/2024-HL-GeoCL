...
def train():
    ...
    val_correl = np.min(
        [pearsonr(preds[2][:, i], targs[2][:, i])[0] for i in range(preds[0].shape[-1])]
    )
    train_correl = np.min(
        [pearsonr(preds[0][:, i], targs[0][:, i])[0] for i in range(preds[0].shape[-1])]
    )
    test_correl = np.min(
        [pearsonr(preds[1][:, i], targs[1][:, i])[0] for i in range(preds[0].shape[-1])]
    )
    hse_correl = np.min(
        [pearsonr(preds[3][:, i], targs[3][:, i])[0] for i in range(preds[0].shape[-1])]
    )
