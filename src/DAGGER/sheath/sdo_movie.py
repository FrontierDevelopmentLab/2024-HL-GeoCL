...
cloud_fetcher = CloudFetcher(zarr_bucket="us-fdlx-ard-zarr-synoptic", aia_path = "fdl-sdoml-v2/sdomlv2.zarr",
                 hmi_path = "fdl-sdoml-v2/sdomlv2_hmi.zarr")

for i, timestamp in tqdm(enumerate(image_indices.index[::5])):
    aia_image = cloud_fetcher.load_aia_image(time=timestamp, idxs=image_indices.loc[timestamp])["193A"]
    hmi_image = cloud_fetcher.load_hmi_image(time=timestamp, idx=image_indices.loc[timestamp]["idx_Bx"])["Bz"]
    plt.figure()
    plt.axis('off')
