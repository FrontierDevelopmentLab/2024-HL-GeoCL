...
for month in range(12):
    ...
    if os.path.isfile(
        "/home/ptigkas/data/iaga/{year}/supermag_iaga_{year}_{month}.npz"
    ):
        continue
    ...
    df2 = pd.pivot_table(
        df, values=features, index=["Date_UTC"], columns=["IAGA"], dropna=False
    )
    ...
    data = df2.values.reshape(
        len(dates), len(features), len(df.IAGA.unique())
    ).swapaxes(2, 1)
    ...
    np.savez(
        f"{output_path}/supermag_iaga_{year}_{month}.npz",
        data=data,
        dates=dates,
        columns=df2.columns,
        stations=sorted(df.IAGA.unique()),
    ...
    np.savez(
        f"{output_path}/supermag_iaga_tiny_{year}_{month}.npz",
        data=data[:1000],
        dates=dates,
        columns=df2.columns,
        stations=sorted(df.IAGA.unique()),
