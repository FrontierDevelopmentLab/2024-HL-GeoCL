...
class SHEATH:
    ...
    def __init__(self, preprocessor, model, solarwind_param_list, scaler_y=None):
        ...
        self.model = model
        self.solarwind_param_list = (
            solarwind_param_list  # List of solar wind parameters
        )
    ...
    def predict(self, inputdatacube):
        ...
        solar_wind = pd.DataFrame.from_dict(
            {
                param: model_output[:, i]
                for i, param in enumerate(self.solarwind_param_list)
            }
        )
        return solar_wind
