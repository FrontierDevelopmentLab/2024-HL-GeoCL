...
class Preprocessor_CH:
    ...
    def preprocess(self, aia_data, hmi_data, aia_wavelengths, hmi_components):
        ...
        mask = get_mask(n_size=n_size)[
            :, int(n_size // 2) - npix : int(n_size // 2) + npix
        ]
        if self.ch_mask:
            segmentation = subsample_and_segment(
                aia_data["193A"][:, int(n_size // 2) - npix : int(n_size // 2) + npix],
                mask,
                region=["CH"],
                ncomp=3,
            )
            ...
            segmentation = np.ones_like(
                aia_data["193A"][:, int(n_size // 2) - npix : int(n_size // 2) + npix]
            )

        aia_data = np.asarray(
            [
                aia_data[channel][:, int(n_size // 2) - npix : int(n_size // 2) + npix]
                for channel in aia_data.keys()
            ]
        ) * (segmentation[None, ...])
        hmi_data = np.asarray(
            [
                hmi_data[comp][:, int(n_size // 2) - npix : int(n_size // 2) + npix]
                for comp in hmi_data.keys()
            ]
        ) * (segmentation[None, ...])
