...
def extract_timestamp_from_filename(filename):
    ...
    timestamp_str = filename.split('/')[-1].split('.')[0][-12:]  # Extract the timestamp part
    timestamp = datetime.datetime.strptime(timestamp_str, '%Y%m%d%H%M')  # Parse the timestamp
...
def get_dagger():
    ...
    last_uploaded_prediction, last_uploaded_prediction_time = get_last_uploaded_object('hl-geo', 'india-jackson-1', 'uncertainty_vm_test/predictions')
    last_uploaded_input, last_uploaded_input_time = get_last_uploaded_object('hl-geo', 'india-jackson-1','uncertainty_vm_test/inputs')

    last_uploaded_dbe_mean_graph, last_uploaded_dbe_mean_graph_time = get_last_uploaded_object('hl-geo', 'india-jackson-1','uncertainty_vm_test/graphs/dbe_geo_mean')
    last_uploaded_dbn_mean_graph, last_uploaded_dbn_mean_graph_time = get_last_uploaded_object('hl-geo', 'india-jackson-1','uncertainty_vm_test/graphs/dbn_geo_mean')
    last_uploaded_dbe_sd_graph, last_uploaded_dbe_sd_graph_time = get_last_uploaded_object('hl-geo', 'india-jackson-1', 'uncertainty_vm_test/graphs/dbe_geo_sd')
    last_uploaded_dbn_sd_graph, last_uploaded_dbn_sd_graph_time = get_last_uploaded_object('hl-geo', 'india-jackson-1', 'uncertainty_vm_test/graphs/dbn_geo_sd')

    inputs_data = requests.get(f"https://storage.googleapis.com/india-jackson-1/{last_uploaded_input}", allow_redirects=True)
    predictions_data = requests.get(f"https://storage.googleapis.com/india-jackson-1/{last_uploaded_prediction}", allow_redirects=True)
    ...
    print(f"The most recently uploaded object is: {last_uploaded_prediction} at {last_uploaded_prediction_time}")
    print(f"The most recently uploaded object is: {last_uploaded_input} at {last_uploaded_input_time}")
