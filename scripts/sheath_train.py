...
def train():
    ...
    scaler_dir = "/home/bjha/data/geocloak/formatted_data/sheath_scaler"
    ...
    for epoch in range(wandb.config.epochs):
        ...
        for inputs, targets in tq.tqdm(train_loader, desc="Train", ascii=" =>"):
        ...
        run = 0
        with torch.no_grad():
            for inputs, targets in tq.tqdm(val_loader, desc="Valid", ascii=" =>"):
                inputs, targets = inputs.to(device), targets.to(device)
                outputs = model(inputs)
                loss = criterion(outputs, targets)
                val_loss += loss.item()
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project="sheath_SGD")
