...
##############
class Dipole(object):
    def __init__(self, epoch=2020.0, dipole_pole=None, B0=None):
        ...
        if dipole_pole is None and B0 is None:  # Calculate dipole parameters from IGRF
            ...
            self.string_representation = "Dipole object for epoch {}".format(
                str(date[0].date())
            )
            ...
            self.string_representation = "Dipole object, user-specified pole at {} degrees and B0 = {} nT".format(
                dipole_pole, B0
            )
    ...
    def mlon2mlt(self, mlon, times):
        ...
        mlt = (180.0 + londiff) / 15.0  # convert to mlt with ssqlon at noon
    ...
    def get_apex_base_vectors(self, lat, r, R=6371.2):
        ...
        if np.any(r / np.cos(la) ** 2 < R):
            raise ValueError(
                "get_apex_base_vectors: Some points have apex height < R. Apex height is r/cos(lat)**2."
            )
        ...
        d1 = R2r ** (3.0 / 2) * np.vstack((np.ones(N), np.zeros(N), np.zeros(N)))
        d2 = (
            -(R2r ** (3.0 / 2))
            / C
            * np.vstack((np.zeros(N), 2 * np.sin(la), np.cos(la)))
        )
        d3 = (
            R2r ** (-3)
            * C
            / (4 - 3 * np.cos(la) ** 2)
            * np.vstack((np.zeros(N), np.cos(la), -2 * np.sin(la)))
        )
    ...
    def map_vperp(self, lon, lat, alt, v, alt_target, R=6371.2):
        ...
        _1, _2, _3, e1, e2, e3 = self.get_apex_base_vectors_geo(
            lon_target, lat_target, r_target, R=R
        )
    ...
    def get_flux_numerical(self, lon, lat, dlon=1.0, dlat=0.1, R=6371.2):
        ...
        boundary = np.interp(
            lonxx, lon, lat, period=360
        )  # interpolate input to constant step size

        minlat = lat.min()
        latxx = np.r_[90 - dlat / 2 : minlat : -dlat][
            ::-1
        ]  # latitude integration steps

        d1, d2, d3, e1, e2, e3 = self.get_apex_base_vectors(
            latxx, R, R=R
        )  # get Apex base vectors
        ...
        sinIm = (
            2
            * np.sin(np.deg2rad(latxx))
            / np.sqrt(4 - 3 * np.cos(np.deg2rad(latxx)) ** 2)
        )
        dF = (
            (R * 1e3) ** 2
            * np.cos(np.deg2rad(latxx))
            * np.abs(sinIm)
            * Be3
            * np.deg2rad(dlon)
            * np.deg2rad(dlat)
        )  # flux per lon and lat according to Richmond95 Eq 4.15
        dF = np.tile(dF, (lonxx.size, 1))
        dF[latxx.reshape((1, -1)) < boundary.reshape((-1, 1))] = (
            0  # mask elements equatorward of boundary
        )
...
if __name__ == "__main__":
    ...
    assert np.allclose(
        np.linalg.norm(np.cross(e3.T, BB.T), axis=1), 0
    )  # e3 perpendicular to B
    assert np.allclose(
        np.linalg.norm(np.cross(d3.T, BB.T), axis=1), 0
    )  # d3 perpendicular to B
    assert np.all(np.sum(d3 * BB, axis=0) > 0)  # e3 along B
    assert np.all(np.sum(e3 * BB, axis=0) > 0)  # d3 along B

    # test scaling properties
    la = -np.linspace(1e-3, 0.7 * np.pi / 2, 100)
    req = 10 * R
    r = req * np.cos(la) ** 2
    ...
    Be3 = (
        d3[1] * Bn + d3[2] * Br
    )  # should be constant since all d3 are on same field line
    ...
    gla, glo, gAe, gAn = d.mag2geo(
        lat, lon, Ae=np.ones(np.sum(iii)), An=np.zeros(np.sum(iii))
    )
    print(d)
